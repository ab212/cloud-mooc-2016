...
public class SearchEngineTester {
	...
	public static void searchKeyword(String keyword) throws Exception {
		...
		for (KeyValue kv : indexRow.list()) {
			...
			freq = Bytes.toInt(freqBytes);

			// Step 2: get the URI of the page from clueWeb09DataTable
			Get uri = new Get(pageDocIdBytes);
			uri.addColumn(Constants.CF_DETAILS_BYTES, Constants.QUAL_URI_BYTES);	
			Result uriRow = dataTable.get(uri);
			byte[] uriBytes = uriRow.getValue(Constants.CF_DETAILS_BYTES, Constants.QUAL_URI_BYTES);
			pageUri = Bytes.toString(uriBytes);

			// Step 3: get the page rank value of this page from clueWeb09PageRankTable
			Get gPageRank = new Get(pageDocIdBytes);
			Result resPageRank = prTable.get(gPageRank);
			byte[] pageRankBytes = resPageRank.list().get(0).getQualifier();
			pageRank = Bytes.toFloat(pageRankBytes);
			// END of your code
